...
def logged_in_client(client):
    client.post("/api/auth/signup", json={"email": "jobsuser@example.com", "password": "securepass123"})
    client.post("/api/auth/login", json={"email": "jobsuser@example.com", "password": "securepass123"})
    return client
...
def test_create_job(logged_in_client):
    response = logged_in_client.post("/api/jobs", json={"company": "Google", "role": "SOC Analyst"})
...
def test_create_job_invalid_status(logged_in_client):
    response = logged_in_client.post("/api/jobs", json={"company": "Google", "role": "SOC", "status": "bogus"})
    assert response.status_code == 400

def test_update_job_status(logged_in_client):
    create_response = logged_in_client.post("/api/jobs", json={"company": "Google", "role": "SOC Analyst"})
...
def test_invalid_deadline_format(logged_in_client):
    response = logged_in_client.post("/api/jobs", json={"company": "Google", "role": "SOC", "deadline": "not-a-date"})
    assert response.status_code == 400
