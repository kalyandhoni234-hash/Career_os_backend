...
def upsert_profile():
    ...
    for field in ["education", "degree", "graduation_year", "country", "preferred_roles", "skills", "experience", "languages", "interests", "preferred_locations", "salary_expectation"]:
        if field in data:
            setattr(profile, field, data[field])

    resume_fields = ["full_name", "phone", "location", "title", "website", "linkedin", "github"]
    if resume:
...
def dashboard_summary():
    ...
    profile_fields = ["education", "degree", "graduation_year", "country", "preferred_roles", "skills", "experience", "languages", "interests", "preferred_locations", "salary_expectation"]
    filled = sum(1 for f in profile_fields if getattr(profile, f, None)) if profile else 0
    ...
    upcoming_deadlines = sorted(
        [
            {"company": j.company, "role": j.role, "deadline": j.deadline.isoformat(), "id": j.id, "status": j.status}
            for j in jobs
    ...
    for job in sorted(jobs, key=lambda j: j.updated_at or j.created_at, reverse=True)[:5]:
        desc = f"Applied to {job.company} for {job.role}" if job.status == "applied" else f"{job.status.capitalize()} at {job.company} for {job.role}"
        recent_activity.append({
            "type": "job",
            "description": desc,
            "timestamp": (job.updated_at or job.created_at).isoformat(),
            "metadata": {"job_id": job.id, "status": job.status, "company": job.company, "role": job.role}
        })

    recent_messages = CoachMessage.query.filter_by(user_id=current_user.id, role="user").order_by(CoachMessage.created_at.desc()).limit(3).all()
    ...
    return jsonify({
        "email": current_user.email,
        "name": resume.full_name if resume and resume.full_name else current_user.email.split("@")[0],
...
def career_overview():
    ...
    if resume:
        ...
        for ach in (resume.achievements or []):
            milestones.append({
                "type": "achievement",
                "title": ach if isinstance(ach, str) else ach.get("title", "Achievement"),
                "subtitle": ach.get("description", "") if isinstance(ach, dict) else "",
    ...
    for job in jobs:
        milestones.append({
            ...
            "date": (job.updated_at or job.created_at).strftime("%Y-%m-%d") if (job.updated_at or job.created_at) else "",
        })
    ...
    return jsonify({
        "full_name": resume.full_name if resume else current_user.email.split("@")[0],
        "milestones": milestones[:50],
        "years_active": years_active,
        "total_roles": total_roles,
        "skills_gained": sorted(skills_set),
        "top_achievement": resume.achievements[0] if resume and resume.achievements else None,
    }), 200
