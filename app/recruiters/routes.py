...
def recruiter_signup():
    ...
    recruiter = Recruiter(user_id=user.id, company_id=company.id, full_name=full_name, title=title)
...
def company_crud():
    ...
    data = request.get_json(silent=True) or {}
    for field in ("website", "logo_url", "description", "industry", "headquarters",
                  "company_size", "founded_year", "linkedin_url", "twitter_url",
                  "culture_description", "benefits_description"):
        if field in data:
...
def search_candidates():
    ...
    min_ats = request.args.get("min_ats", type=int)
    min_career_score = request.args.get("min_career_score", type=int)
    min_github_score = request.args.get("min_github_score", type=int)
    min_projects = request.args.get("min_projects", type=int)
    min_certifications = request.args.get("min_certifications", type=int)
    ...
    recently_active = request.args.get("recently_active", type=bool)
    has_resume = request.args.get("has_resume", type=bool)
    has_portfolio = request.args.get("has_portfolio", type=bool)
    open_source = request.args.get("open_source", type=bool)
    open_to_work = request.args.get("open_to_work", type=bool)
    graduation_year = request.args.get("graduation_year", type=int)
    expected_salary_min = request.args.get("expected_salary_min", type=int)
    expected_salary_max = request.args.get("expected_salary_max", type=int)
    ...
    if branches:
        for branch in branches:
            query = query.filter(Resume.education.astype(db.String).ilike(f"%{branch}%"))

    if locations:
        query = query.filter(Resume.location.in_(locations))

    if remote is not None:
        query = query.filter(Profile.preferred_locations.astype(db.String).ilike("%remote%") if remote else ~Profile.preferred_locations.astype(db.String).ilike("%remote%"))
    ...
    total = query.count()
    results = query.order_by(User.created_at.desc()).offset((page - 1) * per_page).limit(per_page).all()
...
def candidate_detail(candidate_id):
    ...
    recruiter = _get_recruiter()
    if recruiter:
        view = CandidateView(recruiter_id=recruiter.id, candidate_id=candidate_id, source="profile_view")
        db.session.add(view)
        db.session.commit()
    ...
    score_snap = CareerScoreSnapshot.query.filter_by(user_id=candidate_id).order_by(CareerScoreSnapshot.created_at.desc()).first()
    ...
    if profile:
        ...
        data.update({k: None for k in ("education", "degree", "graduation_year", "country", "preferred_roles", "skills", "experience", "languages", "interests", "preferred_locations", "salary_expectation")})
...
def candidate_ai_summary(candidate_id):
    ...
    if resume.projects:
        proj_texts = [f"- {p.get('name', '')}: {p.get('description', '')[:100]}" for p in resume.projects[:5]]
        context_parts.append("Projects:\n" + "\n".join(proj_texts))
    if resume.education:
        edu_texts = [f"{e.get('degree', '')} in {e.get('field', '')} at {e.get('school', '')}" for e in resume.education[:2]]
...
def save_candidate():
    ...
    existing = SavedCandidate.query.filter_by(recruiter_id=recruiter.id, candidate_id=candidate_id).first()
    if existing:
        return jsonify({"error": "Candidate already saved"}), 409

    saved = SavedCandidate(recruiter_id=recruiter.id, candidate_id=candidate_id, pipeline_id=pipeline_id, notes=notes)
...
def unsave_candidate(saved_id):
    ...
    saved = SavedCandidate.query.filter_by(id=saved_id, recruiter_id=recruiter.id).first()
    if not saved:
...
def create_pipeline():
    ...
    pipeline = TalentPipeline(recruiter_id=recruiter.id, name=name, description=data.get("description"), color=data.get("color"))
    db.session.add(pipeline)
    db.session.commit()

    return jsonify({"message": "Pipeline created", "pipeline": {
        "id": pipeline.id, "name": pipeline.name, "description": pipeline.description,
        "color": pipeline.color, "candidate_count": 0,
...
def pipeline_crud(pipeline_id):
    ...
    pipeline = TalentPipeline.query.filter_by(id=pipeline_id, recruiter_id=recruiter.id).first()
    if not pipeline:
...
def list_job_posts():
    ...
    jobs = JobPost.query.filter_by(recruiter_id=recruiter.id).order_by(JobPost.created_at.desc()).all()
    results = []
    for job in jobs:
...
def update_job_post(job_id):
    ...
    data = request.get_json(silent=True) or {}
    text_fields = ("title", "description", "location", "experience_required", "experience_max", "employment_type", "salary_currency", "status")
    ...
    if data.get("application_deadline"):
        job.application_deadline = datetime.fromisoformat(data["application_deadline"]).date()
    db.session.commit()
...
def list_invites():
    ...
    invites = InterviewInvite.query.filter_by(recruiter_id=recruiter.id).order_by(InterviewInvite.created_at.desc()).all()
    results = []
    for inv in invites:
...
def update_invite(invite_id):
    ...
    invite = InterviewInvite.query.filter_by(id=invite_id, recruiter_id=recruiter.id).first()
    if not invite:
        return jsonify({"error": "Invite not found"}), 404

    data = request.get_json(silent=True) or {}
    for field in ("message", "interview_type", "location", "status", "duration_minutes"):
        if field in data:
...
def list_notifications():
    ...
    notifications = query.order_by(RecruiterNotification.created_at.desc()).limit(50).all()
...
def mark_notifications_read():
    ...
    if notification_ids:
        ...
        RecruiterNotification.query.filter_by(recruiter_id=recruiter.id, is_read=False).update(
...
def recruiter_dashboard():
    ...
    active_jobs = JobPost.query.filter_by(recruiter_id=recruiter.id, status="active").count()

    saved_count = SavedCandidate.query.filter_by(recruiter_id=recruiter.id).count()

    recent_applications = SavedCandidate.query.filter_by(recruiter_id=recruiter.id).order_by(
        SavedCandidate.created_at.desc()
    ).limit(10).all()
    ...
    for inv in InterviewInvite.query.filter_by(recruiter_id=recruiter.id).order_by(
            InterviewInvite.created_at.desc()).limit(5).all():
        candidate = User.query.get(inv.candidate_id)
        name = candidate.email.split("@")[0] if candidate else "Unknown"
        recent_activity.append({
            "type": "invite",
...
def recruiter_analytics():
    ...
    top_colleges = db.session.query(
        Profile.education, db.func.count(Profile.id).label("count")
    ).filter(Profile.education.isnot(None)).group_by(Profile.education).order_by(
        db.desc("count")
    ).limit(10).all()
    ...
    return jsonify({
        ...
        "top_skills": [{"name": s[0], "count": s[1]} for s in top_skills_sorted[:10]],
    }), 200
...
def compare_candidates():
    ...
    for cid in candidate_ids:
        ...
        career_score = None
        from app.career.models import CareerScoreSnapshot
        snap = CareerScoreSnapshot.query.filter_by(user_id=cid).order_by(CareerScoreSnapshot.created_at.desc()).first()
        if snap:
...
def contact_candidate(candidate_id):
    ...
    if action == "save":
        ...
        saved = SavedCandidate(recruiter_id=recruiter.id, candidate_id=candidate_id, status="contacted")
        db.session.add(saved)
        db.session.commit()
        return jsonify({"message": "Candidate saved", "saved_id": saved.id}), 201

    if action == "reject":
        saved = SavedCandidate.query.filter_by(recruiter_id=recruiter.id, candidate_id=candidate_id).first()
        if saved:
