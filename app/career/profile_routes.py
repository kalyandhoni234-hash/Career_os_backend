    ...
    return [
        {
            ...
            "cgpa": r.cgpa,
            "relevant_coursework": r.relevant_coursework or [],
...
def _social_links_data(user_id):
    ...
    return [{"id": link.id, "platform": link.platform, "url": link.url} for link in links]


def _resume_files_data(user_id):
    files = (
        ResumeFile.query.filter_by(user_id=user_id, is_active=True)
...
def save_wizard_step(step):
    ...
    try:
        if step == "basic_info":
            ...
            cp.preferred_industry = data.get(
                "preferred_industry", getattr(cp, "preferred_industry", "")
            )
            cp.preferred_country = data.get(
                "preferred_country", getattr(cp, "preferred_country", "")
            )
            cp.work_preference = data.get(
                "work_preference", getattr(cp, "work_preference", "")
            )
        ...
        return jsonify(
            {"message": "Step saved", "completion_pct": _completion_pct(uid)}
        ), 200

    except Exception as e:
        db.session.rollback()
        logger.error(
            "Failed to save wizard step %s for user %s: %s",
            step,
            uid,
            str(e),
            exc_info=True,
...
def create_education():
    ...
    try:
        max_order = (
            db.session.query(db.func.max(UserEducation.order))
        ...
        return jsonify(
            {
                ...
                "current_semester": edu.current_semester,
                "cgpa": edu.cgpa,
...
def reorder_education():
    ...
    for edu_id, order in order_map.items():
        edu = UserEducation.query.filter_by(
            id=int(edu_id), user_id=current_user.id
        ).first()
        if edu:
            edu.order = order
    db.session.commit()
    ...
    return jsonify(
        {
            ...
            "confidence_rating": skill.confidence_rating,
        }
    ), 201
...
def delete_interest(interest_id):
    interest = UserInterest.query.filter_by(
        id=interest_id, user_id=current_user.id
    ).first()
    if not interest:
    ...
    try:
        if section == "basic_info":
            ...
            if not cp:
                ...
                db.session.add(cp)
            mapping = {
                "dream_role": "target_role",
                "dream_company": "target_company",
                "salary_goal": "target_salary",
            }
            for key, val in fields.items():
                col = mapping.get(key, key)
                setattr(cp, col, val)
...
def _check_and_log_profile_events(user_id):
    ...
    if profile:
        if profile.first_name or profile.last_name or profile.country:
            _log_timeline_event(
                ...
                "profile",
                "Profile Created",
                "Basic information added to your career profile",
                2,
            )
    if cp:
        if cp.target_role:
            _log_timeline_event(
                user_id,
                "goal",
                "Dream Career Selected",
                f"Target role set to {cp.target_role}",
                3,
            )
        if cp.career_level:
            _log_timeline_event(
                ...
                f"Current status: {cp.career_level}",
                2,
            )
    ...
    if edu_count > 0:
        last_edu = (
            UserEducation.query.filter_by(user_id=user_id)
            .order_by(UserEducation.created_at.desc())
            .first()
        )
        _log_timeline_event(
            user_id,
            "education",
            "Education Added",
            f"{last_edu.degree} at {last_edu.institution}"
            if last_edu
            else "Education record added",
            2,
        )
    ...
    if skill_count > 0:
        _log_timeline_event(
            ...
            "learning",
            "Skills Added",
            f"{skill_count} skill(s) added to your profile",
            2,
        )
    resume_count = ResumeFile.query.filter_by(user_id=user_id, is_active=True).count()
    if resume_count > 0:
        _log_timeline_event(
            user_id, "resume", "Resume Uploaded", "Resume file added to your profile", 3
        )
...
def career_readiness_score():
    ...
    try:
        ...
        resume_score = (
            100
            if ResumeFile.query.filter_by(user_id=uid, is_active=True).count() > 0
            else 0
        )
...
def career_summary():
    ...
    try:
        ...
        education = (
            UserEducation.query.filter_by(user_id=uid)
        ...
        if cp and cp.career_level:
            level_map = {
                ...
                "intern": "Currently an intern",
                "fresher": "A recent graduate",
...
def smart_recommendations():
    ...
    has_dream = (
        has_dream_role and has_dream_role.target_role and has_dream_role.target_company
    )
        ...
        recs.append(
            {
                ...
                "description": "Helps AI analyze your background and match you with opportunities",
                "icon": "FileText",
                "priority": 1,
            }
        )
        ...
        recs.append(
            {
                ...
                "description": "Your educational background helps personalize career recommendations",
                "icon": "GraduationCap",
                "priority": 2,
            }
        )
        ...
        recs.append(
            {
                "id": "add_skills",
                "action": "Add Skills",
                "description": "Skills power your career insights and skill gap analysis",
                "icon": "Zap",
                "priority": 3,
            }
        )
        ...
        recs.append(
            {
                ...
                "description": "Link your GitHub to showcase projects and coding activity",
                "icon": "Github",
                "priority": 4,
            }
        )
        ...
        recs.append(
            {
                ...
                "description": "Goals help Career OS build a personalized roadmap for you",
                "icon": "Target",
                "priority": 5,
            }
        )
    if not has_dream:
        recs.append(
            {
                "id": "set_dream_career",
                "action": "Set Dream Career",
                "description": "Define your dream role and company for tailored recommendations",
                "icon": "Star",
                "priority": 6,
            }
        )
