...
def ensure_default_agents(user_id):
    ...
    for agent_type in AGENT_TYPES:
        existing = CareerAgent.query.filter_by(user_id=user_id, agent_type=agent_type).first()
        if not existing:
    ...
    db.session.commit()
    return CareerAgent.query.filter_by(user_id=user_id).order_by(CareerAgent.agent_type).all()


def get_agent_statuses(user_id):
    """Return all agents with their current status for a user."""
    agents = CareerAgent.query.filter_by(user_id=user_id).order_by(CareerAgent.agent_type).all()
    if not agents:
        agents = ensure_default_agents(user_id)
    result = []
    for a in agents:
        pending_tasks = AgentTask.query.filter_by(agent_id=a.id, status="pending").count()
        running_tasks = AgentTask.query.filter_by(agent_id=a.id, status="running").count()
        result.append({
            "id": a.id,
...
def get_recent_tasks(user_id, limit=20):
    ...
    tasks = AgentTask.query.filter_by(user_id=user_id)\
...
def run_agent(user_id, agent_type):
    ...
    try:
        ...
        logger.error("Agent %s failed for user %s: %s", agent_type, user_id, str(e), exc_info=True)
        task.status = "failed"
...
def _run_ats_intelligence(user_id):
    ...
    return {"ats_score": result.get("overall_score"), "matched": len(result.get("matched", [])), "missing": len(result.get("missing", []))}
...
def _run_opportunity_ranking(user_id):
    ...
    for s in saved:
        ...
        ranked.append({"opportunity_id": s.opportunity_id, "score": score.get("overall_score") if score else 0})
...
def _run_interview_preparation(user_id):
    ...
    generated = 0
    for s in saved:
        existing = InterviewPack.query.filter_by(opportunity_id=s.opportunity_id, user_id=user_id).first()
        if not existing:
            try:
...
def get_agent_briefing(user_id):
    ...
    # ── Hero Stats ──
    total_opportunities = Opportunity.query.filter(Opportunity.created_at >= week_ago).count()
    ...
    if resume:
        ...
        latest_version = ResumeVersion.query.filter_by(resume_id=resume.id)\
    ...
    # Career Score
    score_snapshot = CareerScoreSnapshot.query.filter_by(user_id=user_id)\
        .order_by(CareerScoreSnapshot.created_at.desc()).first()
    current_score = score_snapshot.overall_score if score_snapshot else 0

    # Score change today
    yesterday_snapshot = CareerScoreSnapshot.query.filter_by(user_id=user_id)\
        .filter(CareerScoreSnapshot.created_at < today_start)\
        .order_by(CareerScoreSnapshot.created_at.desc()).first()
    score_change = current_score - (yesterday_snapshot.overall_score if yesterday_snapshot else current_score)

    # Interview probability (estimated from top match scores & interview packs)
    top_scores = OpportunityMatchScore.query.filter_by(user_id=user_id)\
        .order_by(OpportunityMatchScore.overall_score.desc()).limit(5).all()
    avg_top_match = sum(s.overall_score for s in top_scores) / max(len(top_scores), 1)
    interview_probability_current = min(95, int(avg_top_match * 0.4 + (interview_packs_count * 5)))
    ...
    for a in agents_data:
        descs = running_descriptions.get(a["agent_type"], [f"{agent_labels.get(a['agent_type'], a['agent_type'])} running"])
        ...
        agent_activities.append({
            ...
            "status": a["status"],
            "action": action,
            "progress": a.get("running_tasks", 0) * 33 if a["status"] == "running" else 0,
    ...
    today_tasks = AgentTask.query.filter(
        AgentTask.user_id == user_id,
        AgentTask.status == "completed",
        AgentTask.completed_at >= today_start,
    ).order_by(AgentTask.completed_at.desc()).limit(20).all()
    ...
    for t in today_tasks:
        timeline_events.append({
            ...
            "category": t.task_type.split("_")[0] if "_" in t.task_type else "agent",
        })

    # From career timeline events this week
    career_events = CareerTimelineEvent.query.filter_by(user_id=user_id)\
        .filter(CareerTimelineEvent.event_date >= week_ago)\
        .order_by(CareerTimelineEvent.event_date.desc()).limit(10).all()
    for e in career_events:
        icon_map = {
            "resume": "FileText", "application": "Briefcase", "interview": "MessageSquare",
            "skill": "Zap", "score": "TrendingUp", "goal": "Target",
        }
        timeline_events.append({
    ...
    recent_opps = Opportunity.query.filter(Opportunity.created_at >= week_ago)\
    ...
    market_trend = MarketTrend.query.filter_by(trend_type="trending")\
        .order_by(MarketTrend.created_at.desc()).first()
    if market_trend:
        highlights.append({
            "type": "insight",
    ...
    recs = AIRecommendation.query.filter_by(user_id=user_id, is_dismissed=False, is_completed=False)\
        .order_by(AIRecommendation.priority.desc(), AIRecommendation.impact_score.desc()).limit(5).all()
    for r in recs:
        recommendations.append({
            ...
            "impact": f"+{r.impact_score}% improvement expected" if r.impact_score else "Positive career impact",
            "confidence": min(99, max(60, r.impact_score * 10)) if r.impact_score else 75,
        })
    ...
    if not recommendations:
        ...
        if resume and resume.skills:
            marketing = analyze_skills_gaps_local(resume.skills or [], "Backend Engineer")
    ...
    if resume:
        ...
        fields = [resume.full_name, resume.email, resume.summary, resume.experience, resume.education, resume.skills, resume.projects]
    ...
    learning_progress = int(sum(lp.proficiency for lp in learning_items) / max(len(learning_items), 1)) if learning_items else 0
    ...
    ai_memory = {
        ...
        "preferred_locations": cp.preferred_locations if cp and cp.preferred_locations else [],
    ...
    # ── Opportunity Feed ──
    opportunity_feed = []
    opportunity_feed_raw = OpportunityMatchScore.query.filter_by(user_id=user_id)\
        .order_by(OpportunityMatchScore.overall_score.desc()).limit(8).all()
    seen_ids = set()
    ...
    from app.jobs.models import Job
    apps_this_week = Job.query.filter(Job.user_id == user_id, Job.created_at >= last_week_start).count()
    ...
    # Career score growth
    last_week_score_snapshot = CareerScoreSnapshot.query.filter_by(user_id=user_id)\
        .filter(CareerScoreSnapshot.created_at < last_week_start)\
        .order_by(CareerScoreSnapshot.created_at.desc()).first()
    last_week_score = last_week_score_snapshot.overall_score if last_week_score_snapshot else 0

    weekly_progress = {
        "applications": {"current": apps_this_week, "last_week": apps_last_week},
        "resume_updates": {"current": resume_improvements, "last_week": max(0, resume_improvements - 1)},
        "skills_learned": {"current": skills_this_week, "last_week": skills_last_week},
        "interviews": {"current": sum(1 for t in today_tasks if "interview" in t.task_type), "last_week": 0},
        "career_score_growth": {"current": current_score, "last_week": last_week_score},
        "ats_growth": {"current": career_health["ats_score"], "last_week": max(0, career_health["ats_score"] - 5)},
    }
    ...
    return {
        ...
        "hero_stats": {
            ...
            "interview_probability": {"current": interview_probability_current, "change": interview_probability_predicted - interview_probability_current},
        },
        "daily_brief": {
...
def analyze_skills_gaps_local(skills, target_role):
    ...
    common_gaps = {
        "Backend Engineer": [("Docker", 64), ("Redis", 58), ("PostgreSQL", 72),
                            ("Kubernetes", 48), ("AWS", 55), ("System Design", 45)],
