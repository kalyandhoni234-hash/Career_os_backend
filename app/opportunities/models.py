...
class SavedOpportunity(db.Model):
    """Tracks a user's interest in an Opportunity and application state."""
    ...
    application_status = db.Column(db.String(50), default="saved")
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    updated_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(timezone.utc),
...
class ResumeVersionByCompany(db.Model):
    """Snapshot of a resume tailored for a specific company/opportunity."""
    ...
    company_name = db.Column(db.String(255), nullable=False)
    resume_snapshot = db.Column(db.JSON, nullable=False)
    ...
    opportunity = db.relationship(
        "Opportunity", backref=db.backref("resume_versions_by_company", lazy="dynamic")
    )
